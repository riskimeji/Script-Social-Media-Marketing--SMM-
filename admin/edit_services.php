...
$user = mysqli_query($conn, "SELECT * FROM service where id = '$id'");
...
if(isset($_POST['submit'])){
   ...
   $update = mysqli_query($conn, "UPDATE service set category = '$kategori', nama ='$nama', deskripsi = '$desk', p_id ='$p_id', status ='$status', harga= '$harga', min = '$min', max= '$max', est_masuk = '$est' where id ='$id'");
   if($update){
...
<html lang="en" dir="ltr">
  <head>
    <meta charset="utf-8">
      ...
      <title>Edit Service | Meji SMM</title>
...
?>
                ...
                <div class="form-group">
                            ...
                            <input type="text" name="nama" class="form-control" id="email1" value="<?php echo $array['nama']?>" placeholder="nama layanan.." required>
                        </div>
                        <div class="form-group">
                            <label class="form-label" for="email">Provider ID:</label>
                            <input type="number" name="p_id" value="<?php echo $array['p_id']?>" class="form-control" id="email1" placeholder="1234" required>
                        </div>
                        <div class="form-group">
                        <div class="row">
                           <div class="col">
                            ...
                            <div class="input-group has-validation">
                              ...
                              <input type="number" name="harga" value="<?php echo $array['harga']?>" class="form-control" id="price" placeholder="0" aria-describedby="inputGroupPrepend" required>
                           </div>  
                           </div>
                            <div class="col">
                            <label class="form-label" for="email">Minimal Pesan</label>
                            <input type="number" id="min" name="min" value="<?php echo $array['min']?>" class="form-control" placeholder="0" required>
                            </div>
                            <div class="col">
                            <label class="form-label" for="email">Maximal Pesan:</label>
                            <input type="number" id="max" name="max" value="<?php echo $array['max']?>" class="form-control" placeholder="0" required>
                            </div>
                            <div class="col">
                            <label class="form-label" for="email">Waktu Rata:</label>
                            <input type="text" id="average"  value="<?php echo $array['est_masuk']?>" name="est_masuk" class="form-control" required>
                            </div>
                        </div>
                        </div>
                        <div class="form-group">
                        <label class="form-label" for="email">Deskripsi:</label>
                           <textarea id="deskripsi" name="deskripsi" class="form-control" required><?php echo $array['deskripsi']?></textarea>
                        </div>
                        <div class="form-group">
